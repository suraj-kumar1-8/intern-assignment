const KEY = "fairshare-v1";

function hydrate(data) {
  return {
    groupName: data.groupName,
    members: (data.members || []).map((m) => ({ ...m })),
    expenses: (data.expenses || []).map((e) => ({
      ...e,
      date: typeof e.date === "string" ? e.date : e.date instanceof Date ? e.date.toISOString().slice(0, 10) : e.date,
    })),
  };
}

export function loadState(seed) {
  try {
    const raw = localStorage.getItem(KEY);
    if (!raw) {
      const initial = hydrate(seed);
      localStorage.setItem(KEY, JSON.stringify(initial));
      return initial;
    }
    return hydrate(JSON.parse(raw));
  } catch {
    return hydrate(seed);
  }
}

export function persistState(state) {
  localStorage.setItem(KEY, JSON.stringify(state));
}

export function nextExpenseId() {
  return `e-${Date.now()}`;
}

export function nextMemberId(members) {
  const max = (members || []).reduce((m, x) => (Number(x.id) > m ? Number(x.id) : m), 0);
  return max + 1;
}

export function reducer(state, action) {
  switch (action.type) {
    case "ADD_EXPENSE": {
      return { ...state, expenses: [action.expense, ...state.expenses] };
    }

    case "DELETE_EXPENSE": {
      const next = action.id !== undefined
        ? state.expenses.filter((e) => e.id !== action.id)
        : state.expenses.filter((_, idx) => idx !== action.index);
      return { ...state, expenses: next };
    }
    case "UPDATE_EXPENSE": {
      const next = action.id !== undefined
        ? state.expenses.map((e) => (e.id === action.id ? { ...e, ...action.patch } : e))
        : state.expenses.map((e, idx) => (idx === action.index ? { ...e, ...action.patch } : e));
      return { ...state, expenses: next };
    }
    case "ADD_MEMBER": {
      return { ...state, members: [...state.members, action.member] };
    }
    default:
      return state;
  }
}

